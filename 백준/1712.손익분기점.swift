...
어느 순간 총 수입(판매비용)이 총 비용(=고정비용+가변비용)보다 많아지게 된다. 최초로 총 수입이 총 비용보다 많아져 이익이 발생하는 지점을 손익분기점이라고 한다.
...
var x : Double = -a[0] / (a[1] - a[2])

if x > 0 {
    ceil(x) > x ? print(Int(ceil(x))) : print(Int(x) + 1)
} else {
    print("-1")
}
